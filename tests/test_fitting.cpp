...
void test_curve_fit(Bezier<Scalar, dim, degree, generic> curve, int num_tests = 10, int num_control_pts = degree+1)
{
...
template <int dim, int degree, bool generic>
void test_curve_fit(BSpline<Scalar, dim, degree, generic> curve, int num_control_pts, int num_tests = 10)
{
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        auto computed_curve =
            BSpline<Scalar, dim, degree, false>::fit(parameter_values, curve_samples, num_control_pts, knots);
        assert_same(computed_curve, true_curve, num_samples, 0., 1., 0., 1., 1e-5);
    }
}
template <int dim, int degree_u, int degree_v>
void test_patch_fit(BezierPatch<Scalar, dim, degree_u, degree_v> curve, int num_tests = 5)
{
    for (int i = 0; i < num_tests; i++) {
...
template <int dim, int degree_u, int degree_v>
void test_patch_fit(BSplinePatch<Scalar, dim, degree_u, degree_v> curve, int num_control_pts_u,  int num_control_pts_v, int num_tests = 5)
{
    //const int num_control_pts = degree + 1;
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        auto computed_patch = BSplinePatch<Scalar, dim, degree_u, degree_v>::fit(
                parameter_values, patch_samples, num_control_pts_u, num_control_pts_v,
                knots_u, knots_v);
        assert_same(computed_patch, true_patch, num_samples, 0., 1., 0., 1., 0., 1., 0., 1.,1e-5);
    }
}
...
TEST_CASE("test least squares fitting for Bezier curves", "[nonrational][bezier][fit]"){
    ...
    SECTION("random polynomial fitting: dim 10, degree 10"){
        Bezier<Scalar, 10, 10, false> curve;
        test_curve_fit(curve);
    }
}
...
TEST_CASE("test least squares fitting for BSpline curves", "[nonrational][bspline][fit]"){
    SECTION("random polynomial fitting: dim 2, degree 4"){
        ...
        test_curve_fit(curve,12);
        test_curve_fit(curve,18);
    }
    SECTION("random polynomial fitting: dim 3, degree 15"){
        BSpline<Scalar, 3, 15, false> curve;
        test_curve_fit(curve, 30);
    }
    SECTION("random polynomial fitting: dim 10, degree 10"){
        BSpline<Scalar, 10, 10, false> curve;
        test_curve_fit(curve,15);
    } 
}
...
TEST_CASE("test least squares fitting for Bezier patches", "[nonrational][bezier_patch][fit]"){
    ...
    SECTION("random polynomial fitting: dim 2, high mixed degree"){
...
TEST_CASE("test least squares fitting for BSpline patches", "[nonrational][bspline_patch][fit]"){
    ...
    SECTION("random polynomial fitting: dim 2, degree 4"){
        ...
        test_patch_fit(patch,5,12);
        test_patch_fit(patch,13,5);
    }
    SECTION("random polynomial fitting: dim 3, mixed degree"){
        BSplinePatch<Scalar, 3, 9, 7> patch;
        test_patch_fit(patch,15, 14);
    }
}
...
TEST_CASE("simple test for deformations to test for seg faults", "[deform]"){
    ...
    SECTION("Curves"){
        ...
        Eigen::MatrixXd parameter_values = 
            .5*(Eigen::MatrixXd::Random(num_samples, 1) + 
                    Eigen::MatrixXd::Constant(num_samples, 1, 1.));
        ...
        SECTION("Bezier"){
            ...
            REQUIRE(new_control_points.isApprox(control_pts + Eigen::MatrixXd::Constant(num_control_pts, dim, 1)));
        }
        SECTION("BSpline"){
    ...
    SECTION("Patches"){
        ...
        Eigen::MatrixXd parameter_values = 
            .5*(Eigen::MatrixXd::Random(num_samples, 2) + 
                    Eigen::MatrixXd::Constant(num_samples, 2, 1.));
        Eigen::MatrixXd deformations = Eigen::MatrixXd::Constant(num_samples, dim, 1.);
