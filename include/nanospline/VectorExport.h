            ...
            int num_samples = 1e3)
{
    ...
    for (int i = 0; i < num_control_points; i++)
    {
        ...
        out << "<circle cx=\"" << (p[0] + offset[0]) * scaling
            << "\" cy=\"" << (p[1] + offset[1]) * scaling << "\" r=\""
            << ctrl_radius * scaling << "\"/>" << std::endl;
    }
    out << "</g>" << std::endl;
...
template <typename SplineType, typename Matrix>
void to_eps(std::ostream &out, SplineType &curve, const Matrix &offset, typename SplineType::Scalar scaling = 1,
            bool export_ctrl = true,
            const std::string &color = "255 0 0", typename SplineType::Scalar line_width = 0.2,
            const std::string &ctrl_color = "88 88 88", typename SplineType::Scalar ctrl_radius = 1,
            int num_samples = 1e3)
{
    //TODO: maybe approximate this curve into many cubic beziers
    using Scalar = typename SplineType::Scalar;
    ...
    for (int i = 0; i < num_samples + 1; i++)
    {
        ...
        if (i == 0)
        {
            out << (p[0] + offset[0]) * scaling << " " << (p[1] + offset[1]) * scaling << " moveto\n";
        }
        else
        {
            out << (p[0] + offset[0]) * scaling << " " << (p[1] + offset[1]) * scaling << " lineto\n";
        }
    }
    out << "stoke" << std::endl;
    ...
    {
        const auto &p = control_points.row(i);
        out << (p[0] + offset[0]) * scaling << " "
            << (p[1] + offset[1]) * scaling << " "
