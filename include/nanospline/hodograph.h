...
Bezier<Scalar, dim, (degree <= 0 ? degree : degree - 1), generic> compute_hodograph(
    const Bezier<Scalar, dim, degree, generic>& curve)
{
    ...
    typename HodographType::ControlPoints ctrl_pts_2(num_ctrl_pts - 1, ctrl_pts.cols());
...
template <typename Scalar, int dim, int degree, bool generic>
BSpline<Scalar, dim, (degree <= 0 ? degree : degree - 1), generic> compute_hodograph(
