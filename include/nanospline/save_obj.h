...
int export_patch_obj(std::ofstream& fout,
    const PatchType& patch,
    const int num_u_samples,
    const int num_v_samples,
    const int offset)
{
    ...
    for (int ui = 0; ui < num_u_samples - 1; ui++) {
        for (int vi = 0; vi < num_v_samples - 1; vi++) {
            ...
            fout << "f " << c0 + offset + 1 << " " << c1 + offset + 1 << " " << c2 + offset + 1
...
void save_patch_obj(const std::string& filename,
    const PatchType& patch,
    int num_samples_u = 100,
    int num_samples_v = 100)
{
    std::ofstream fout(filename.c_str());
...
template <typename PatchType>
void save_patch_obj(const std::string& filename, const std::vector<PatchType>& patches)
{
    std::ofstream fout(filename.c_str());
    int count = 0;
